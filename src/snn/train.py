...
def train_classification(model: nn.Module,
...
def train_triplet(model: nn.Module,
                  train_loader: torch.utils.data.DataLoader,
                  device: torch.cuda.device = None,
                  optimizer: torch.optim.Optimizer = None,
                  criterion: Callable = None,
...
class Trainer():
    def __init__(self, loss: Union[Callable, str], optimizer, cuda: bool, log_interval: int = 100, metrics: Sequence = None):
        if type(loss) is str:
            if loss == 'contrastive':
        ...
        if isinstance(self.loss, nn.BCELoss):
            self.training_epoch = partial(train_classification,
                  device = self.device,
            ...
            self.training_epoch = partial(train_contrastive,
                  device = self.device,
                  optimizer = self.optimizer,
                  criterion = self.loss,
                  log_interval = self.log_interval)
            ...
            self.training_epoch = partial(train_triplet,
                  device = self.device,
                  optimizer = self.optimizer,
                  criterion = self.loss,
                  log_interval = self.log_interval)              
            ...
            raise Exception('Loss function provided to Trainer constcrutor is not explicitly supported')
                        
                        
    def train(self, model: nn.Module, train_loader, scheduler, val_loader = None, start_epoch: int = 0, n_epochs: int = 20, dry_run: bool = False):
        """
        ...
        for epoch in range(start_epoch, n_epochs):
            ...
            train_loss = self.training_epoch(model, 
                                             train_loader, 
                                             epoch=epoch, 
                                             dry_run = dry_run)
            if dry_run:
