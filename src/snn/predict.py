...
def predict_distances(model: torch.nn.Module, dataset: ContrastiveDataset, device: torch.cuda.device = None) -> list:
    """
    ...
    with torch.no_grad():
        ...
        for (img1, img2), target in dataset:
            ...
            dist = np.linalg.norm(embedding_1.squeeze().cpu().numpy() - embedding_2.squeeze().cpu().numpy())
