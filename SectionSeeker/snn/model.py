        ...
        if self.config.BACKBONE_MODEL_WEIGHTS:
            ...
            try:
                ...
                self.backbone = get_model(
                    self.config.BACKBONE_MODEL, weights=model_weights
                )
                weights_loaded = True
            except ValueError:
                self.backbone = get_model(self.config.BACKBONE_MODEL)
                self.backbone.load_state_dict(
                    torch.load(self.config.BACKBONE_MODEL_WEIGHTS)
                )
                weights_loaded = True
...
class ClassificationNet(nn.Module, PyTorchModelHubMixin):
    """
    ...
    def __init__(self, embedding_net: nn.Module):
        ...
        self.classifier = nn.Sequential(
            nn.ReLU(inplace=True), nn.Linear(self.embedding_net.latent_space_dim * 2, 1)
        )
